...
#include "NuiApi.h"
#undef __No__NuiSkeleton_h__


// Default Kinect intrinsic camera parameters, normalized by image size, 
// based on Kinect for Windows SDK values
#define NUI_KINECT_DEPTH_NORM_FOCAL_LENGTH_X (NUI_CAMERA_DEPTH_NOMINAL_FOCAL_LENGTH_IN_PIXELS/320.0f)
#define NUI_KINECT_DEPTH_NORM_FOCAL_LENGTH_Y (NUI_CAMERA_DEPTH_NOMINAL_FOCAL_LENGTH_IN_PIXELS/240.0f)
...
#define NUI_FUSION_DEFAULT_INTEGRATION_WEIGHT (150)
...
///       <description>The <paramref name="pImageFrame"/> parameter's <c>pFrameTexture</c> member
///       is NULL.</description>
///    </item>
///    <item>
///       <term>E_FAIL</term>
...
///       parameter is NULL, 
///       the <paramref name="minDepth"/> parameter or <paramref name="maxDepth"/> is less 
...
///    </item>
///       <term>E_NOINTERFACE</term>
///       <description>The <paramref name="pDepthFloatFrame"/> parameter's <c>pFrameTexture</c> 
...
///       <description>A memory allocation failed on the GPU.</description>
///    </item>
///    <item>
///       <term>E_NUI_FEATURE_NOT_INITIALIZED</term>
///       <description>The Kinect camera Runtime could not be accessed.</description>
...
_Check_return_ HRESULT FUSIONAPI NuiFusionDepthToDepthFloatFrame(
    _In_ const NUI_DEPTH_IMAGE_PIXEL *pDepthImageData,
...
///       <paramref name="pPointCloudFrame"/> parameter's <c>pFrameTexture</c> member is NULL.
///       </description>
///    </item>
///    <item>
///       <term>E_OUTOFMEMORY</term>
...
/// world to camera transform (camera pose) to be used in lighting calculations. Pass identity if
/// the point cloud did not originate from a raycast and is in the camera local coordinate system.
/// </param>
/// <param name="pWorldToBGRTransform">Optionally, a pointer to a pre-allocated Matrix4, which contains
/// a mapping from world co-ordinates to an RGB color space. If valid, this transform is used to
/// color surfaces according to their world positions. RGB values are clamped to the range [0, 1].
/// If this parameter is NULL, then a simpler shading routine is used that does not include 
/// colored surface rendering.</param>
...
///       parameter's <c>pFrameTexture</c> member is NULL.</description>
///    </item>
///    <item>
///       <term>E_OUTOFMEMORY</term>
///       <description>A CPU memory allocation failed.</description>
...
/// frame, to be filled with color-coded data from the camera tracking. Values vary depending on
/// whether the pixel was a valid pixel used in tracking (green) or failed in different tests. 
/// Pass NULL if not required.</param>
/// <param name="pObservedToReferenceTransform">A pointer to a pre-allocated transformation matrix.
/// At entry to the function this should be filled with the best guess for the observed to
/// reference transform (usually the last frame's calculated pose). At exit this is filled with the
...
///       <paramref name="pObservedPointCloudFrame"/> or <paramref name="pObservedToReferenceTransform" />
...
///       <paramref name="pDataAssociationTestResultsFrame"/> parameter's <c>pFrameTexture</c> 
///       member is NULL.</description>
///    </item>
///    <item>
...
///       <paramref name="pObservedToReferenceTransform"/> parameter is set to identity.</description>
///    </item>
///    <item>
///       <term>E_OUTOFMEMORY</term>
///       <description>A CPU memory allocation failed.</description>
...
_Check_return_ HRESULT FUSIONAPI NuiFusionAlignPointClouds(
    ...
    _Inout_ Matrix4 *pObservedToReferenceTransform
    );
