...
#define INuiFusionReconstruction_ProcessFrame(This,pDepthFloatFrame,maxAlignIterationCount,maxIntegrationWeight,pWorldToCameraTransform)	\
    ( (This)->lpVtbl -> ProcessFrame(This,pDepthFloatFrame,maxAlignIterationCount,maxIntegrationWeight,pWorldToCameraTransform) ) 
...
/// This enables a specific device to be chosen when
/// calling NuiFusionCreateReconstruction if desired.
/// </summary>
...
/// or -1 for the default device for the given processor type.</param>
/// <param name="pDescription">An optional pointer to a buffer that receives a description string for the device.</param>
/// <param name="descriptionSizeInChars">The size of the buffer pointed to by <paramref name="pDescription"/>, in characters.</param>
/// <param name="pInstancePath">An optional pointer to a buffer that receives an instance path string for the device.</param>
/// <param name="instancePathSizeInChars">The size of the buffer pointed to by <paramref name="pInstancePath"/>, in characters.</param>
/// <param name="pMemoryKB">An optional pointer to an unsigned integer that receives the total amount of memory on the device, in kilobytes.</param>
/// <returns>
...
/// Users must call Release on the interface when finished with the volume.
/// Voxel volume axis sizes must be greater than 0 and a multiple of 32.
/// Users can select which device the processing is performed on
/// with the <paramref name="reconstructionProcessorType"/>
/// parameter. For those with multiple devices the <paramref name="pDeviceIndex"/> parameter also
/// enables users to explicitly configure on which device the reconstruction volume is created.
...
/// </param>
/// <param name="deviceIndex">Set this variable to an explicit zero-based
/// device index to use a specific device as enumerated by NuiFusionGetDeviceInfo,
/// or set to -1 to automatically select the default device for a given processor type.
/// </param>
/// <param name="pInitialWorldToCameraTransform">
/// Optionally, a pointer to a pre-allocated Matrix4 instance, containing the initial camera pose with
/// respect to the world origin. Pass NULL to set identity (the default initial camera pose).
/// </param>
...
///       available devices for the respective processor type.</description>
///    </item>
///    <item>
///       <term>E_POINTER</term>
